    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <div class="vs-component vs-con-input-label vs-input mt-5 w-full vs-input-primary">
          ...
          <div class="vs-con-input">
            <flat-pickr v-model="dataDate" class="w-full"/>
          </div>
        </div>
  ...
  watch: {
    isSidebarActive (val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        const { _id, name, category, status, sector, date, time, place, organizer, description } = JSON.parse(JSON.stringify(this.data))
        this.dataId = _id
